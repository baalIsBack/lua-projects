local Super = require 'applications.filefighter.BaseWindow'
...
function Self:init(args)
  ...
  self.callbacks:register("onDropped", function(selff, obj, x, y)
    ...
    self.main.explorer:add(obj)
    self.main.desktop.hand = nil
    obj.x = mx - self:getX() + obj._dragOffsetX
    obj.y = my - self:getY() + obj._dragOffsetY
  ...
  self.callbacks:register("update", function(selff, dt)
    self.main.explorer.contents:callall("update", dt)
    --it happens, but the position of the contents seems off, since i cant grab them
    --self:repositionIcons()
  end)
...
--  love.graphics.setStencilTest("greater", 0)
  ...
  self.main.explorer.contents:callall("draw")
