...
function hex_linedraw (a, b)
    ...
    local a_nudge = Hex(a.q + 1e-06, a.r + 1e-06, a.s - 2e-06)
    local b_nudge = Hex(b.q + 1e-06, b.r + 1e-06, b.s - 2e-06)
    local results = {}
...
function polygon_corners_flattened (layout, h)
    local corners = {}
    local center = hex_to_pixel(layout, h)
    for i = 0, 5 do
        local offset = hex_corner_offset(layout, i)
        table.insert(corners, center.x + offset.x)
        table.insert(corners, center.y + offset.y)
    end
    return corners
end
